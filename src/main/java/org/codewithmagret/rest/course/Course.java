...
public class Course {
     ...
     * This field is automatically generated by the database and serves as the primary key for the course.
     */
    @Id
     ...
     * This field is required and represents the name of the course that students will see when browsing courses.
     */
    private String title;

    /**
     * The unique code for the course.
     * This field is required and must be unique across all courses to prevent duplicates.
     * It can be used as a shorthand identifier for the course, such as "CS101" or "MATH202".
     */
    private String code;

    /**
     * Many courses can belong to one category. Foreign key: `category_id`
     * The `@JoinColumn` annotation specifies the foreign key column in the course table that references the category table.
     * The `nullable = false` attribute indicates that every course must belong to a category, enforcing referential integrity.
     */
    @ManyToOne
     ...
     * The `@JoinColumn` annotation specifies the foreign key column in the course table that references the instructor table.
     * The `nullable = false` attribute indicates that every course must have an instructor, enforcing referential integrity.
     */
    @ManyToOne
     ...
     * The `mappedBy` attribute indicates that the `courses` field in the Student entity is responsible for managing this relationship.
     * The @JsonIgnore annotation is used to prevent infinite recursion during JSON serialization, as the Student entity also references the Course.
     */
